...
send(
    p,
    {
        ...
        "params": {
            "processId": os.getpid(),
            "rootUri": u(ROOT),
            "workspaceFolders": [{"uri": u(ROOT), "name": "root"}],
            "capabilities": {},
        },
    },
)
...
for loc in res:
    r = loc["range"]["start"]
    print(
        f"  {loc['uri'].replace('file://' + ROOT + '/', '')}  (line {r['line']}, col {r['character']})"
    )

send(p, {"jsonrpc": "2.0", "id": 3, "method": "shutdown", "params": None})
until(p, 3)
