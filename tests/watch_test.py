...
def completion_labels(p, path, line, character):
    ...
    send(p, {"jsonrpc": "2.0", "method": "textDocument/didOpen", "params": {
        "textDocument": {"uri": u(path), "languageId": "yaml", "version": 1,
                         "text": open(path).read()}}})
    send(p, {"jsonrpc": "2.0", "id": rid, "method": "textDocument/completion", "params": {
        "textDocument": {"uri": u(path)}, "position": {"line": line, "character": character}}})
    res = pump_until_id(p, rid)
    items = res["result"]
    items = items["items"] if isinstance(items, dict) else (items or [])
    send(p, {"jsonrpc": "2.0", "method": "textDocument/didClose",
...
def watched(p, path, typ):
    ...
    send(p, {"jsonrpc": "2.0", "method": "workspace/didChangeWatchedFiles",
...
def main():
    ...
    try:
        ...
        print(f"[{'PASS' if SAW_REGISTER[0] else 'FAIL'}] server registered file watchers")
        ok &= SAW_REGISTER[0]

        print(f"[{'PASS' if 'watched_thing' not in before else 'FAIL'}] var absent before file exists")
        ...
        print(f"[{'PASS' if 'watched_thing' in after else 'FAIL'}] var appears after Created event")
        ...
        print(f"[{'PASS' if 'watched_thing' not in gone else 'FAIL'}] var removed after Deleted event")
